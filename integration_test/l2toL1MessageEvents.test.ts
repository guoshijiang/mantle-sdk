...
describe('L2ToL1Message events', () => {
  ...
  const createProviderMock = async (networkChoiceOverride?: number) => {
    ...
    const latestBlock = l2Network.mantleGenesisBlock + 1000
  ...
  it('does call for mantle events', async () => {
    const { l2Network, l2Provider, l2ProviderMock } = await createProviderMock()
    const fromBlock = l2Network.mantleGenesisBlock
    const toBlock = l2Network.mantleGenesisBlock + 500
  ...
  it('does call for classic and mantle events', async () => {
    const { l2Network, l2Provider, l2ProviderMock } = await createProviderMock()
    const fromBlock = 0
    const toBlock = l2Network.mantleGenesisBlock + 500
    ...
    verify(
      l2ProviderMock.getLogs(
        deepEqual({
          ...
          topics: [classicTopic],
          fromBlock: fromBlock,
          toBlock: l2Network.mantleGenesisBlock,
        })
      )
    ).once()
    verify(
      l2ProviderMock.getLogs(
        deepEqual({
          address: arbSys,
          topics: [mantleTopic],
          fromBlock: l2Network.mantleGenesisBlock,
          toBlock: toBlock,
        })
      )
    ).once()
  ...
  it('does call for classic and mantle events from earliest to latest', async () => {
    const { l2Network, l2Provider, l2ProviderMock } = await createProviderMock()
  ...
  it('does call for only mantle for latest', async () => {
    const { l2Network, l2Provider, l2ProviderMock } = await createProviderMock()
    const fromBlock = l2Network.mantleGenesisBlock + 2
    const toBlock = 'latest'
  ...
  it('doesnt call classic when mantle genesis is 0', async () => {
