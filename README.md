...
Typescript library for client-side interactions with Mantle. Mantle SDK provides common helper functionaliy as well access to the underlying smart contract interfaces.

Below is an overview of the Mantle SDK functionality. See the [tutorials](https://github.com/mantlenetworkio/mantle-tutorials) for further examples of how to use these classes.

### Quickstart Recipes

- ##### Deposit Ether Into Mantle

```ts
import { getL2Network, EthBridger } from '@mantle/sdk'

const l2Network = await getL2Network(
  l2ChainID /** <-- chain id of target Mantle chain */
)
...
```ts
import { L1TransactionReceipt, L1ToL2MessageStatus } from '@mantle/sdk'
...
```ts
import { L2TransactionReceipt } from '@mantle/sdk'

const l2TxnReceipt = new L2TransactionReceipt(
  txnReceipt /** <-- ethers-js TransactionReceipt of an mantle tx */
)
...
Mantle SDK can be used to bridge assets to/from the rollup chain.The following asset bridgers are currently available:
...
Mantle SDK comes pre-configured for the Mainnet and Rinkeby, and their Mantle counterparts. However the networks functionlity can be used register networks for custom Mantle instances. Most of the classes in Mantle SDK depend on network objects so this must be configured before using other Mantle SDK functionlity.

### Inbox tools

As part of normal operation the Mantle sequencer will messages into the rollup chain. However, if the sequencer is unavailable and not posting batches, the inbox tools can be used to force the inclusion of transactions into the rollup chain.
...
- **constants** - A list of useful Mantle related constants

### Run Integration tests

1. First, make sure you have a Mantle node running. Check out the mantle repo, and run the following command `./test-node.bash --init --no-blockscout`
...
`rinkArby` expects env var `MTL_KEY` to be prefunded with at least 0.02 ETH, and env var `INFURA_KEY` to be set.
