...
import { MantleProvider } from '../utils/arbProvider'
 ...
 */
export type L2ToL1MessageReaderOrWriterMantle<T extends SignerOrProvider> =
  T extends Provider ? L2ToL1MessageReaderMantle : L2ToL1MessageWriterMantle
...
/**
 * Base functionality for mantle L2->L1 messages
 */
export class L2ToL1MessageMantle {
  ...
  ): L2ToL1MessageReaderOrWriterMantle<T>
  public static fromEvent<T extends SignerOrProvider>(
    l1SignerOrProvider: T,
    event: EventArgs<L2ToL1TxEvent>
  ): L2ToL1MessageReaderMantle | L2ToL1MessageWriterMantle {
    return SignerProviderUtils.isSigner(l1SignerOrProvider)
      ? new L2ToL1MessageWriterMantle(l1SignerOrProvider, event)
      : new L2ToL1MessageReaderMantle(l1SignerOrProvider, event)
  }
...
/**
 * Provides read-only access mantle for l2-to-l1-messages
 */
export class L2ToL1MessageReaderMantle extends L2ToL1MessageMantle {
  ...
  private async getBlockFromNodeLog(
    ...
    const mantleProvider = new MantleProvider(l2Provider)
    const l2Block = await mantleProvider.getBlock(
...
/**
 * Provides read and write access for mantle l2-to-l1-messages
 */
export class L2ToL1MessageWriterMantle extends L2ToL1MessageReaderMantle {
  constructor(
