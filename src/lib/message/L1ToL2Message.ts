...
import { MtRetryableTx__factory } from '../abi/factories/MtRetryableTx__factory'
...
export abstract class L1ToL2Message {
  ...
  public static calculateSubmitRetryableId(
    ...
    // mantle submit retry transactions have type 0x69
...
export class L1ToL2MessageReader extends L1ToL2Message {
  ...
  public static async getLifetime(l2Provider: Provider): Promise<BigNumber> {
    const arbRetryableTx = MtRetryableTx__factory.connect(
...
export class EthDepositMessage {
  ...
  public static calculateDepositTxId(
    ...
    // https://github.com/mantlenetworkio/go-ethereum/blob/07e017aa73e32be92aadb52fa327c552e1b7b118/core/types/arb_types.go#L302-L308
    const fields = [
      formatNumber(chainId),
      zeroPad(formatNumber(msgNum), 32),
    ...
    // mantle eth deposit transactions have type 0x64
    const rlpEnc = ethers.utils.hexConcat([
      '0x64',
      ethers.utils.RLP.encode(fields),
    ])
  ...
  private static parseEthDepositData(eventData: string): {
    ...
    // https://github.com/mantlenetworkio/mantle/blob/aa84e899cbc902bf6da753b1d66668a1def2c106/contracts/src/bridge/Inbox.sol#L242
