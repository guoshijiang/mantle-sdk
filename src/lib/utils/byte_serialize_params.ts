...
Mantle SDK includes methods for [serializing parameters](https://developer.mantlenetworkio.com/docs/special_features#parameter-byte-serialization) for a solidity method into a single byte array to minimize calldata. It uses the following schema:
...
import { MtAddressTable__factory } from '../abi/factories/MtAddressTable__factory'
import { MtAddressTable } from '../abi/MtAddressTable'
...
export const getAddressIndex = (() => {
  ...
  let arbAddressTable: MtAddressTable | undefined
  ...
  return async (address: string, signerOrProvider: Signer | Provider) => {
    ...
    arbAddressTable =
      arbAddressTable ||
      MtAddressTable__factory.connect(
