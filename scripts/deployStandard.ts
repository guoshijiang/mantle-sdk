...
const main = async () => {
  ...
  const warningTokens = (
    await axios.get(
      'https://raw.githubusercontent.com/mantlenetworkio/arb-token-lists/master/src/WarningList/warningTokens.json'
    )
  ).data
  ...
  if (warningToken) {
    ...
    console.log()
    console.log(
      `${l1TokenAddress} is ${description}; it will likely have unusual behavior when deployed as as standard token to Mantle. It is not recommended that you deploy it. (See https://developer.mantlenetworkio.com/docs/bridging_assets for more info.)`
    )
    console.log()
