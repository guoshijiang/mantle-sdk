...
export const config = {
  arbUrl: process.env['MTL_URL'] as string,
  ethUrl: process.env['ETH_URL'] as string,
  arbKey: process.env['MTL_KEY'] as string,
...
export const getCustomNetworks = async (
  ...
  try {
    deploymentData = execSync(
      'docker exec mantle_sequencer_1 cat /config/deployment.json'
    ).toString()
  } catch (e) {
    deploymentData = execSync(
      'docker exec mantle-sequencer-1 cat /config/deployment.json'
    ).toString()
  }
