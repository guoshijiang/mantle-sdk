...
async function main() {
  ...
  const mantlePath = getPackagePath('@mantlenetworkio/mantle-contracts')
  ...
  // TODO: use `HARDHAT_ARTIFACT_PATH` to write files to mantle sdk instead of the packages themselves.
  ...
  const mantleFiles = glob(cwd, [
    `${peripheralsPath}/build/contracts/!(build-info)/**/+([a-zA-Z0-9_]).json`,
    `${mantlePath}/build/contracts/!(build-info)/**/+([a-zA-Z0-9_]).json`,
  ])

  // TODO: generate files into different subfolders (ie `/mantle/*`) to avoid overwrite of contracts with the same name
  await runTypeChain({
    cwd,
    filesToProcess: mantleFiles,
    allFiles: mantleFiles,
    outDir: './src/lib/abi/',
    target: 'ethers-v5',
  })
