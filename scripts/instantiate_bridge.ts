...
const arbKey = process.env['MTL_KEY'] as string
...
export const instantiateBridge = (
  ...
  if (!l1PkParam && !ethKey) {
    throw new Error('need MTL_KEY var')
  }
  if (!l2PkParam && !arbKey) {
    throw new Error('need MTL_KEY var')
  }
  ...
  const l2Rpc = (() => {
    if (l2NetworkID === 42161)
      return process.env['MTL_ONE_RPC'] || 'https://arb1.mantle.io/rpc'
    if (l2NetworkID === 421611)
      return process.env['RINKARBY_RPC'] || 'https://rinkeby.mantle.io/rpc'
    throw new Error(
